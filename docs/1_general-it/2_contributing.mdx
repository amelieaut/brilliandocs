...
Docker is the best way to develop your apps. If you use Windows, install Docker Desktop, which also sets up WSL. If you use Mac, download Docker from the Docker website. If you use Linux, you're probably already an expert, but if not, Docker's official docs are your friend.
...
- For testing the AWS deployment:
...
After you've made your awesome changes, push your branch, open a pull request and merge it against the main branch. This will trigger the Continuous Deployment (CD) Pipeline, which deploys to AWS (BrillianIdeas, BrillianQuiz, BrillianCRM/ICM, IMBIT-N3xt) or GitHub Pages for BrillianDocs.
